...
4. Return a JSON object with the format including the reasoning and the ID of your choice:
   {
       "reasoning": <string>,
       "top_id": <integer>
   }

# Here are some sampes Examples
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if match:
                ...
                if top_candidate_id in candidate_ids:
                    return top_candidate_id
                else:
                    print(f"Invalid ID {top_candidate_id} not in batch. Retrying ({attempt + 1}/{max_retries})...")
            else:
                print(f"No matched ID found in the response. Retrying ({attempt + 1}/{max_retries})...")
